...
$OutputDir = Join-Path $ScriptDir "eval-results-clue"
...
if (-not (Test-Path $DataRoot)) {
    throw "CLUE data root not found: $DataRoot"
}
...
try {
    foreach ($Task in $Tasks) {
        Write-Host ""
        Write-Host "=== Evaluating decoder on $($Task.Name) ==="
...
Write-Host ""
Write-Host "Decoder CLUE evaluation complete."
