...
def prepare_hanzi_structure(output_dir: pathlib.Path) -> None:
    ...
    rows = []
    for item in ds:
        phenomenon = item.get("condition") or item.get("task") or item.get("meta_structure", "hanzi_structure")
        rows.append({
            "sentence_good": item["sentence_good"],
            "sentence_bad": item["sentence_bad"],
            "UID": phenomenon,
...
def main():
    ...
    parser.add_argument(
        "--only",
        choices=["all", "zhoblimp", "hanzi", "cogbench", "clue"],
        default="all",
        help="Prepare only one evaluation subset. Use 'clue' for encoder fine-tuning evaluation.",
    )
    args = parser.parse_args()

    if args.only in ["all", "zhoblimp"]:
        prepare_zhoblimp(args.output_dir)
    if args.only in ["all", "hanzi"]:
        prepare_hanzi_structure(args.output_dir)
        prepare_hanzi_pinyin(args.output_dir)
    if args.only in ["all", "cogbench"]:
        prepare_cogbench(args.output_dir)
    if args.only in ["all", "clue"]:
